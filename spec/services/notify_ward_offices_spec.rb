...
RSpec.describe NotifyWardOffices, type: :service do
  let(:year) { "2026" }
  ...
  describe '#call' do
    context 'when write is false' do
      ...
      it 'returns test result string with count' do
        expect(subject).to eq("TEST: 2 ward office(s) would be notified")
      end
    end
    ...
    context 'when write is true' do
      ...
      before do
        ...
        allow(mailer_dbl).to receive(:schedules_live).and_return(mailer_dbl)
      ...
      it 'sends an email to each ward office' do
        ...
        expect(mailer_dbl).to have_received(:schedules_live).twice
        expect(mailer_dbl).to have_received(:deliver_later).twice
      end

      it 'returns success result string with count' do
        expect(subject).to eq("SUCCESS: 2 ward office(s) notified")
      end
      ...
      it 'marks notified offices in the CSV' do
        subject
        expect(csv_output).to include(["WARD", "ALDERMAN", "EMAIL", "NOTIFIED"])
    ...
    context 'when a row has already been notified' do
      let(:headers) { ["WARD", "ALDERMAN", "EMAIL", "NOTIFIED"] }
      ...
      context 'with write false' do
        ...
        it 'skips already-notified rows' do
          expect(subject).to eq("TEST: 1 ward office(s) would be notified")
        end

        it 'logs the skipped count' do
          expect { subject }.to output(/1 office\(s\) already notified, skipping/).to_stdout
        end
      end
      ...
      context 'with write true' do
        ...
        it 'preserves existing NOTIFIED flags and marks new ones' do
          subject
          expect(csv_output).to include(["1", "La Spata, Daniel", "Ward01@cityofchicago.org", "true"])
          expect(csv_output).to include(["2", "Hopkins, Brian", "office@aldermanhopkins.com", "true"])
        end
    ...
    context 'when all rows have already been notified' do
      subject { described_class.new(write: true, year: year).call }
      ...
      before do
        allow(CSV).to receive(:read).with(csv_path, headers: true).and_return(
          csv_table(
            ...
            headers: ["WARD", "ALDERMAN", "EMAIL", "NOTIFIED"],
          )
        )
      end
